...
export const EditarObjetos = () => {
  ...
  useEffect(() => {
    const fetchFoto = async () => {
      const url = `https://backend-prueba-apel.onrender.com/api/objetos/icon/${id}`;
  ...
  const guardar_foto = async (e) => {
    ...
    if (datos.status === "success") {
      ...
      const subida2 = await Api(`https://backend-prueba-apel.onrender.com/api/objetos/editar-imagen/${id}`, "POST", formData, true);
      ...
      const pdfSubida2 = await Api(`https://backend-prueba-apel.onrender.com/api/objetos/editar-pdfs/${id}`, "POST", pdfFormData, true);
