...
export const DocumentacionInstitucion = () => {
  ...
  const getTemas = async () => {
    const url = `https://backend-prueba-apel.onrender.com/api/documentacion/listar-temas-instituciones/${id}`;
  ...
  const handleTemaClick = (tema) => {
    navigate(`/admin/documentacion/${id}/${tema}`);
  };
            ...
            {temas.map((tema, index) => {
              return (
                ...
                >
                    <article>
                  <img id='img_temas'
                    src={`https://backend-prueba-apel.onrender.com/imagenes/documentacion/${tema.nombreImagen}`}
                    alt={`${tema.tema} foto`}
                  />
