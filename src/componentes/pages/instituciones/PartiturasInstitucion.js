...
export const PartiturasInstitucion = () => {
  ...
  const getTemas = async () => {
    const url = `https://backend-prueba-apel.onrender.com/api/partituras/listar-temas-instituciones/${id}`;
  ...
  const handleTemaClick = (tema) => {
    navigate(`/admin/partituras/${id}/${tema}`);
  };
            ...
            {temas.map((tema, index) => {
              return (
                ...
                >
                    <article>
                  <img id='img_temas'
                    src={`https://backend-prueba-apel.onrender.com/imagenes/partituras/${tema.fotoAleatoria}`}
                    alt={`${tema.tema} foto`}
                  />
