...
export const DocumentacionInstitucionTema = () => {
  ...
  const getFotos = async () => {
    const url = `https://backend-prueba-apel.onrender.com/api/documentacion/${institucionId}/${tema}`;
  ...
  const handleFotoClick = (fotografia) => {
    navigate(`/admin/documentacion/${fotografia._id}`);
  };
  ...
  const handleEditClick = (event, fotografiaId) => {
    event.stopPropagation();
    navigate(`/admin/editar/documentacion/${fotografiaId}`);
  };
      ...
      <div className='container_fotografia'>
        ...
        <div className='fotografias-container'>
          {fotos.map((fotografia) => {
            ...
            const imageUrl = firstImage ? `https://backend-prueba-apel.onrender.com/imagenes/documentacion/${firstImage}` : '';
