...
export const IconografiaInstitucionTema = () => {
  ...
  const getFotos = async () => {
    const url = `https://backend-prueba-apel.onrender.com/api/iconografia/${institucionId}/${tema}`;
  ...
  const handleFotoClick = (fotografia) => {
    navigate(`/admin/iconografia/${fotografia._id}`);
  };
      ...
      <div className='container_fotografia'>
        ...
        <div className='fotografias-container'>
          {fotos.map((fotografia) => {
            ...
            const imageUrl = firstImage ? `https://backend-prueba-apel.onrender.com/imagenes/iconografia/${firstImage}` : '';
