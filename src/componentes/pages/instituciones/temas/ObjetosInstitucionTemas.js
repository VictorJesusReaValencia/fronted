...
export const ObjetosInstitucionTema = () => {
  ...
  const getFotos = async () => {
    const url = `https://backend-prueba-apel.onrender.com/api/objetos/${institucionId}/${tema}`;
  ...
  const handleFotoClick = (fotografia) => {
    navigate(`/admin/objetos/${fotografia._id}`);
  };
  ...
  const handleEditClick = (event, fotografiaId) => {
    event.stopPropagation();
    navigate(`/admin/editar/objetos/${fotografiaId}`);
  };
      ...
      <div className='container_fotografia'>
        ...
        <div className='fotografias-container'>
          {fotos.map((fotografia) => {
            ...
            const imageUrl = firstImage ? `https://backend-prueba-apel.onrender.com/imagenes/objetos/${firstImage}` : '';
