...
export const MonumentosInstitucionTema = () => {
  ...
  const getFotos = async () => {
    const url = `https://backend-prueba-apel.onrender.com/api/monumentos/${institucionId}/${tema}`;
  ...
  const handleFotoClick = (fotografia) => {
    navigate(`/admin/monumentos/${fotografia._id}`);
  };
  ...
  const handleEditClick = (event, fotografiaId) => {
    event.stopPropagation();
    navigate(`/admin/editar/monumentos/${fotografiaId}`);
  };
      ...
      <div className='container_fotografia'>
        ...
        <div className='fotografias-container'>
          {fotos.map((fotografia) => {
            ...
            const imageUrl = firstImage ? `https://backend-prueba-apel.onrender.com/imagenes/monumentos/${firstImage}` : '';
