...
export const IconografiaInstitucion = () => {
  ...
  const getTemas = async () => {
    const url = `https://backend-prueba-apel.onrender.com/api/iconografia/listar-temas-instituciones/${id}`;
  ...
  const handleTemaClick = (tema) => {
    navigate(`/admin/iconografia/${id}/${tema}`);
  };
            ...
            {temas.map((tema, index) => {
              return (
                ...
                >
                    <article>
                  <img id='img_temas'
                    src={`https://backend-prueba-apel.onrender.com/imagenes/iconografia/${tema.fotoAleatoria}`}
                    alt={`${tema.tema} foto`}
                  />
