...
export const FotografiasInstitucion = () => {
  ...
  const getTemas = async () => {
    const url = `https://backend-prueba-apel.onrender.com/api/fotografia/listar-temas-instituciones/${id}`;
  ...
  const handleTemaClick = (tema) => {
    navigate(`/admin/fotografia/${id}/${tema}`);
  };
            ...
            {temas.map((tema, index) => {
              return (
                ...
                >
                    <article>
                  <img id='img_temas'
                    src={`https://backend-prueba-apel.onrender.com/imagenes/fotografias/${tema.nombreImagen}`}
                    alt={`${tema.tema} foto`}
                  />
